...
class CreateSkillsDef {
    ...
    private List<Proj> saveProjectsDefs(List<Proj> projDefs = null) {
        ...
        if(projToCreate) {
            log.info("Creating [${projToCreate.size()}] project definitions.")
            List<Callable> callables = projToCreate.collect() { Proj proj ->
                { ->
                    try {
            ...
            int numThreads = Math.min(5, projToCreate.size())
           ...
           log.info("All [${projDefs.size()}] project definitions already created.")
        }
    }
